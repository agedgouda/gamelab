<div>
    ...
    <form wire:submit="{{ $isEditMode ? 'update' : 'store' }}"> 
        ...
        @foreach($dateTimes as $index => $dateTime)
            <div class="grid grid-cols-3 {{ $loop->odd ? 'bg-gray-200' : '' }}">
                <div >
                    <div class="flex items-center">
                        {{ \Carbon\Carbon::parse( $dateTime['date'])->format('m/d/Y') }} {{ \Carbon\Carbon::parse( $dateTime['date'])->format('h:i A') }}
                    </div>
                </div>

                <div class="text-sm">
                    @if(isset($dateTime['available']))
                ...
                <div class="text-sm">
                    @if(isset($dateTime['notAvailable']))
        ...
        <div class="flex justify-end space-x-4 mt-4">
            <x-secondary-button wire:click="{{ $isEditMode ? 'cancel' : 'resetForm' }}">
                {{ __('Cancel') }}
            </x-secondary-button>
